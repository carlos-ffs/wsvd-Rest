...
public class DoSubjectSearch_Vx0 {
    ...
    public List<Book> doSubjectSearch(String search_key) {
        ...
        try {
            // Prepare SQL
            PreparedStatement statement = con.prepareStatement("SELECT * FROM tpcw_item, tpcw_author WHERE tpcw_item.i_a_id = tpcw_author.a_id AND tpcw_item.i_subject = ?  ORDER BY tpcw_item.i_title LIMIT 50");
            // Set parameter
